...
// switcher #define SHUNT_FACTOR 22.10121
#define SHUNT_FACTOR 0.381565
#define MAX_AMPS   6 
// analyzer #define XSCALE      8.970
// switcher #define XSCALE      11.478
#define XSCALE      13.148
#define KEY_CAL 2.88  // Keysight output calibration factor
#define YSCALE     16.0
char    dir[DIRSIZE];
int     sd;
int	time=500;
...
//sprintf(command,"output:state on,(@4003,4004)");
sprintf(command,"output:state on,(@4003)");

/* send message to the server PORT on machine HOST */

    if (send(sd, command, strlen(command), MSG_DONTWAIT) == -1) {
       perror("error turning on DAC outputs");
       close(sd);
       exit(1);
       }
    if (send(sd, "\n", 1, MSG_DONTWAIT) == -1) {
...
if(halt_update == 1)
  ...
  //mouse_value=900-current_y;
  
  sprintf(x_command,"source:voltage:level %f, (@4003)",xval/KEY_CAL);
  //sprintf(y_command,"source:voltage:level %f, (@4004)",yval);

//  system(x_command);
//  system(y_command);
  
// send messages to the server PORT on machine HOST 
 
/*                
  if (send(sd, x_command, strlen(x_command), MSG_DONTWAIT) == -1) {
     perror("send");
     close(sd);
     exit(1);
     }
  ...
  if (send(sd, y_command, strlen(y_command), MSG_DONTWAIT) == -1) {
     perror("send");
     close(sd);
     exit(1);
     }
...
send(sd, x_command, strlen(x_command), MSG_DONTWAIT);
send(sd, "\n", 1, MSG_DONTWAIT);
//send(sd, y_command, strlen(y_command), MSG_DONTWAIT);
//send(sd, "\n", 1, MSG_DONTWAIT);
...
/*
void focus_change(w,data,event)
	Widget    w;
	//focus_change_data *data;
	XEvent            *event;
{
	//printf("\nevent focus change type=%d send_event=%d mode=%d detail=%d\n",event->type,event->send_event,event->mode,event->detail);
	if(event->type == 9)
	  {
	...
	XEnterWindowEvent   *event;
{
	//printf("\nevent focus change type=%d mode=%d detail=%d same_screen=%d focus=%d\n",event->type,event->mode,event ->detail,event->same_screen, event->focus);
        ...
        if((event->type == 7) && (event->mode == 0) && (sd == 0))
      	{
		...
		XLeaveWindowEvent   *event;
{
	...
	//printf("\nevent focus change type=%d \n",event->type);		
       	if((event->type == 8) && (event->mode == 0) && (sd != 0))
                {
                //printf("closeing port %d\n",sd);
                if(close(sd) == -1) perror("close");
                sd = 0;
        }
}
        ...
        XmAnyCallbackStruct     *call_data;
{
        ...
        case 3: /* Disarm */
		   ...
		   */
		   xval = save_xval;
		   yval = save_yval;
                   if ((input = fopen("inflection.tandem.data","w")) != NULL) {
                      fprintf(input,"%s %d %f %f\n","inflection",1,xval*SHUNT_FACTOR,yval);
                      fclose (input);
                      }
                }
        ...
        XmAnyCallbackStruct     *call_data;
{
        ...
        case 3: /* Disarm */
                if (flag == 1) {        /* finished with things, now exit */
                   if(magnet_lock == 1) magnet_lock = 0;
                   else magnet_lock = 1;
                }
...
// 	if (mousemoved == 0) updatedata(&plot);
	...
	*/
	if (rubberband == 1) time = 050;
	     ...
	     if ((input = fopen("inflection.tandem.data","r")) != NULL) {
		...
		//send(sd, "\n", 1, MSG_DONTWAIT);
	     ...
	     if ((input = fopen("inflection.tandem.data","w")) != NULL) {
	        //fprintf(input,"%s %d %f %f\n","inflection",0,xval*3.0,yval*3.0);
                fprintf(input,"%s %d %f %f\n","inflection",0,xval*SHUNT_FACTOR,yval);
	        fclose (input);
	        }
             }
...
/*	printf("start = %d   stop = %d   max = %d\n", plot.start, plot.stop, plot.max); */
  ...
  if(rtype == 0)
    ...
    if(new_width > size_thresh)
      ...
      //sprintf(strbuff,"Switcher = %6.4f Amps port=%d",xval*SHUNT_FACTOR,sd);
      //xlabel
      sprintf(strbuff,"1");
  ...
  if((rtype == 0) && (plot.max > 8))
    ...
    sprintf(strbuff,"Inflection Magnet Current = %7.4f Amps",SHUNT_FACTOR*xval);
